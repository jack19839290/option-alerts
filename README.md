...
- 每次正式收盤更新會在「收盤Delta趨勢」保存 `|Delta|` 0.10、0.15、0.20 最接近但不超過目標的合約；`ALL` 分別保存 CALL 與 PUT。
- 收盤趨勢採每日唯一快照，不由手動更新寫入；無符合合約、Greeks 不足或抓取失敗也保留狀態列，避免缺資料被誤認為 0。
...
- 不會刪除或重新命名任何分頁。
- 會新增「收盤Delta趨勢」分頁及完整欄位與格式。
- 既有掃描設定、期權鍊與警示紀錄不會被刪除。
- 第一筆歷史資料會從升級後下一次成功的正式收盤更新開始累積，不會回推不存在的歷史報價。
